...
2. ARP and IPv6 ND move from Go into the dataplane: plugin-local
...
3. gNMI operational-state read path on the daemon.
4. Plugin suite rework toward generic UP building blocks. The old
...
5. Multi-instance management design ADR: instances stay autonomous;
...
6. RFC-audit the DHCP/PPPoE/L2TP handlers against the corpus in
   references/: every behavioral branch cites its section; fix where
   the text disagrees.
7. Audit FRR's gRPC northbound on the pinned release: how much of
...
8. LCP echo offload into the osvbng_pppoe plugin: answer peer
   ...
   open while writing.
9. Mine the legacy context repo's DECISIONS files lazily: when a
...
10. Hot-path debt recorded in design/performance-and-logging.md:
    ...
    - DHCPv6 packet pipeline parses each packet three times
      ...
      contention
11. NUMA-aware CPU placement on multi-socket hosts: resolution is
...
12. Directions distilled from the CUPS design study, carried here
    by substance:
